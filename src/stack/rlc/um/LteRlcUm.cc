...
UmTxQueue* LteRlcUm::getTxBuffer(MacNodeId nodeId, LogicalCid lcid)
{
    ...
    if(it == txBuffers_.end()) {    // Not found: create
...
UmRxQueue* LteRlcUm::getRxBuffer(MacNodeId nodeId, LogicalCid lcid)
{
    ...
    if(it == rxBuffers_.end()) {    // Not found: create
        std::stringstream buf;
...
void LteRlcUm::initialize() {
    up_[IN] = gate("UM_Sap_up$i"); up_[OUT] = gate("UM_Sap_up$o");
    down_[IN] = gate("UM_Sap_down$i"); down_[OUT] = gate("UM_Sap_down$o");
