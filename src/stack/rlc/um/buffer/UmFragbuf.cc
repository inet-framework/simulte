...
bool UmFragbuf::insert(unsigned int pktId, unsigned int totFrag,
    ...
    debugCounter_++;
//    std::cout << "debugCounter is now " << debugCounter_ << endl;

    // Add fragment to buffer
    if(frags_.find(pktId) == frags_.end()) {                // No fragments for this Main packet
        MainPktInfo_ pkt;
        ...
        return true;
    } else if (frags_[pktId].bitmap_[fragSno] == false) {    // Set this fragment if not a duplicate fragment
...
bool UmFragbuf::check(unsigned int pktID) {
    if(frags_[pktID].fragsleft_ == 0) {        // All fragments received
        return true;
    } else {
        return false;
    }
...
std::ostream &operator<<(std::ostream &stream, const UmFragbuf* fragbuf) {
    if (fragbuf == NULL) return (stream << "Empty set");
    ...
    for(it = frags.begin(); it != frags.end(); it++) {
        ...
        for(unsigned int i = 0; i< it->second.bitmap_.size(); i++) {
            if (it->second.bitmap_[i] == false) stream << "0";
            else stream << "1";
        }
        stream << ";  ";
    }
