...
protected:
    ...
    void generateBaseFeedback(int numBands,int numPreferredBabds, LteFeedback& fb, FeedbackType fbType, int cw,RbAllocationType rbAllocationType,TxMode txmode, std::vector<double> snr);
    // Get cqi from BLer Curves
    Cqi getCqi(TxMode txmode,double snr);
    double meanSnr(std::vector<double> snr);
public:
    LteFeedbackComputationRealistic(double targetBler, std::map<MacNodeId,Lambda>* lambda,double lambdaMinTh,double lambdaMaxTh,double lambdaRatioTh,unsigned int numBands);
