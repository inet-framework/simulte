...
public:
    ...
    LteSummaryFeedback(unsigned char cw, unsigned int b, omnetpp::simtime_t lb,
            omnetpp::simtime_t ub) {
        totCodewords_ = cw;
        logicalBandsTot_ = b;
    ...
    void reset() {
        ...
        cqi_ = std::vector<CqiVector>(totCodewords_,
                CqiVector(logicalBandsTot_, NOSIGNALCQI)); // XXX DUMMY VALUE USED FOR TESTING: replace with NOSIGNALCQI
        tCqi_ = std::vector<std::vector<omnetpp::simtime_t> >(totCodewords_,
                std::vector<omnetpp::simtime_t>(logicalBandsTot_,
                        omnetpp::simTime()));

        pmi_ = PmiVector(logicalBandsTot_, NOPMI);
        tPmi_ = std::vector<omnetpp::simtime_t>(logicalBandsTot_,
                omnetpp::simTime());
        valid_ = false;
    }
     ...
     */
    void print(MacCellId cellId, MacNodeId nodeId, const Direction dir,
            TxMode txm, const char* s) const;
};
