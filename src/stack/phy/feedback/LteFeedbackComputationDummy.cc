...
LteFeedbackDoubleVector LteFeedbackComputationDummy::computeFeedback(FeedbackType fbType, RbAllocationType rbAllocationType,
        TxMode currentTxMode, std::map<Remote,int> antennaCws, int numPreferredBands,  FeedbackGeneratorType feedbackGeneratortype, int numRus,std::vector<double> snr,MacNodeId id)
{
    ...
    for (int j=0;j<numRus;j++){
        ...
        for (int z=0;z<DL_NUM_TXMODE;z++){
            ...
            generateBaseFeedback(numBands_,numPreferredBands, fb, fbType, antennaCws[(Remote)j], rbAllocationType,(TxMode)z);
...
LteFeedbackVector LteFeedbackComputationDummy::computeFeedback(const Remote remote, FeedbackType fbType, RbAllocationType rbAllocationType,
        TxMode currentTxMode, int antennaCws, int numPreferredBands,  FeedbackGeneratorType feedbackGeneratortype, int numRus,std::vector<double> snr,MacNodeId id)
{
    // New Feedback
...
LteFeedback LteFeedbackComputationDummy::computeFeedback(const Remote remote, TxMode txmode, FeedbackType fbType, RbAllocationType rbAllocationType,
...
void LteFeedbackComputationDummy::generateBaseFeedback(int numBands,int numPreferredBands,
    ...
    else if (fbType==PREFERRED){ //TODO:: DA VERIFICARE DA VERIFICARE E COMMENTARE
        CqiVector tmp;
