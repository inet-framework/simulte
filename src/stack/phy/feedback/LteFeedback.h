...
public:
    ...
    bool isEmptyFeedback() const {return !(status_);}
    //! Return true if is periodic.
    bool isPeriodicFeedback() const    {return periodicFeedback_;}

    /// check existence of elements
    bool hasRankIndicator() const {return (status_ & RANK_INDICATION);}
    bool hasWbCqi() const {return (status_ & WB_CQI);}
    bool hasWbPmi() const {return (status_ & WB_PMI);}
    bool hasBandCqi() const {return (status_ & BAND_CQI);}
    bool hasBandPmi() const {return (status_ & BAND_PMI);}
    bool hasPreferredCqi() const {return (status_ & PREFERRED_CQI);}
    bool hasPreferredPmi() const {return (status_ & PREFERRED_PMI);}


    // **************** GETTERS ****************
    //! Get the rank indication. Does not check if valid.
    Rank getRankIndicator() const {return rank_;}
    //! Get the wide-band CQI. Does not check if valid.
    CqiVector getWbCqi() const {return wideBandCqi_;}
    //! Get the wide-band CQI for one codeword. Does not check if valid.
    Cqi getWbCqi(Codeword cw) const {return wideBandCqi_[cw];}
    //! Get the wide-band PMI. Does not check if valid.
    Pmi getWbPmi() const {return wideBandPmi_;}
    //! Get the per-band CQI. Does not check if valid.
    std::vector<CqiVector> getBandCqi() const {return perBandCqi_;}
    //! Get the per-band CQI for one codeword. Does not check if valid.
    CqiVector getBandCqi (Codeword cw) const {return perBandCqi_[cw];}
    //! Get the per-band PMI. Does not check if valid.
    PmiVector getBandPmi() const {return perBandPmi_;}
    //! Get the per preferred band CQI. Does not check if valid.
    CqiVector getPreferredCqi() const {return preferredCqi_;}
    //! Get the per preferred band CQI for one codeword. Does not check if valid.
    Cqi getPreferredCqi(Codeword cw) const {return preferredCqi_[cw];}
    //! Get the per preferred band PMI. Does not check if valid.
    Pmi getPreferredPmi() const {return preferredPmi_;}
    //! Get the set of preferred bands. Does not check if valid.
    BandSet getPreferredBands() const {return preferredBands_;}
    //! Get the transmission mode.
    TxMode getTxMode() const {return txMode_;}
    //! \test DAS support - Get the remote Antenna identifier
    Remote getAntennaId() const {return remoteAntennaId_;}
    // *****************************************
    ...
    void setWideBandCqi(const Cqi cqi,const Codeword cw) {
        if(wideBandCqi_.size() == cw) wideBandCqi_.push_back(cqi);
        else wideBandCqi_[cw] = cqi;
    ...
    void setPerBandCqi(const CqiVector bandCqi,const Codeword cw) {
        if(perBandCqi_.size() == cw) perBandCqi_.push_back(bandCqi);
        else perBandCqi_[cw] = bandCqi;
    ...
    void setPreferredCqi (const Cqi preferredCqi,const Codeword cw) {
        if(preferredCqi_.size() == cw) preferredCqi_.push_back(preferredCqi);
        else preferredCqi_[cw] = preferredCqi;
    ...
    void setTxMode(const TxMode txMode) {txMode_ = txMode;}

    //! \test DAS support - Set the remote Antenna identifier
    void setAntenna(const Remote antenna) {remoteAntennaId_ = antenna;}

    //! Set the periodicity type.
    void setPeriodicity(const bool type) {periodicFeedback_ = type;}
    // *****************************************
...
protected:
    ...
    double confidence(simtime_t creationTime) const
    {
        ...
        if(delta < confidenceLowerBound_) return 1.0;
        if(delta > confidenceUpperBound_) return 0.0;
...
public:
    ...
    void reset()
    {
        ...
        tCqi_ = std::vector< std::vector<simtime_t> >(totCodewords_, std::vector<simtime_t>(logicalBandsTot_, simTime()));
    ...
    void setCqi(Cqi cqi, Codeword cw, Band band) { // note: it is impossible to receive cqi == 0!
        cqi_[cw][band] = cqi;
...
public:
    ...
    LteSummaryBuffer(unsigned char dim, unsigned char cw, unsigned int b, simtime_t lb, simtime_t ub) : cumulativeSummary_(cw, b, lb, ub)
    {
        bufferSize_ = dim;
        totCodewords_ = cw;
        totBands_ = b;
