...
class LteMacPdu : public LteMacPdu_Base
{
    ...
    public:
        ...
        LteMacPdu(const LteMacPdu& other) : LteMacPdu_Base() {
            operator=(other);
        }
        ...
        virtual void setSduArraySize(unsigned int size) { ASSERT(false); }
        virtual unsigned int getSduArraySize() const { return sduList_.size(); }
        virtual cPacket& getSdu(unsigned int k) { MacSduList::iterator it = sduList_.begin(); advance(it, k); return **it; }
        virtual void setSdu(unsigned int k, const cPacket& sdu) { ASSERT(false); }
